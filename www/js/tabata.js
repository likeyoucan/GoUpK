...
export const tb = {
  ...
  init() {
    ...
    this.selectWorkout(
      this.workouts.find((w) => String(w.id) === String(lastSelectedId))
        ? lastSelectedId
        : this.workouts[0].id,
    );
    this.renderList();
    ...
    this.els.list?.addEventListener("click", (e) => {
      ...
      if (delBtn) {
        e.stopPropagation();
        this.deleteWorkout(delBtn.dataset.id);
      } else if (editBtn) {
        e.stopPropagation();
    ...
    this.els.list?.addEventListener("keydown", (e) => {
      ...
      if (e.key === "Enter" && row) this.selectWorkout(row.dataset.id);
    });
  ...
  openModal(idToEdit = null) {
    ...
    if (idToEdit) {
      const w = this.workouts.find((x) => String(x.id) === String(idToEdit));
      if (w) {
    ...
    this.els.modal.classList.remove("hidden", "translate-y-full");
  ...
  saveWorkout() {
    let finalName =
      this.els.editName.value.trim() || this.getUniqueName(t("tabata"));
    const exists = this.workouts.some(
      (w) =>
        w.name.toLowerCase() === finalName.toLowerCase() &&
        String(w.id) !== String(this.editingWorkoutId),
    );
    if (exists) {
    ...
    const work = Math.max(1, parseInt(this.els.editWork.value) || 20),
      rest = Math.max(1, parseInt(this.els.editRest.value) || 10),
      rounds = Math.max(1, parseInt(this.els.editRounds.value) || 8);
    if (this.editingWorkoutId) {
      const index = this.workouts.findIndex(
        (x) => String(x.id) === String(this.editingWorkoutId),
      );
      if (index > -1)
        this.workouts[index] = {
          ...this.workouts[index],
  ...
  deleteWorkout(id) {
    ...
    const deletedIndex = this.workouts.findIndex(
  ...
  renderList() {
    ...
    this.workouts.forEach((w) => {
      ...
      const isAct = String(w.id) === String(this.selectedId);
      div.tabIndex = 0;
  ...
  start() {
    ...
    this.currentRound = 1;
    this.status = "READY";
    this.phaseDuration = READY_DURATION_MS;
  ...
  tick(isBackground = false) {
    ...
    if (rem <= 0) {
      this.nextPhase(rem < -2000 ? Math.abs(rem) : 0);
      return;
    }
  ...
  nextPhase(missedTime = 0) {
    ...
    if (missedTime > 0) {
      ...
      while (remMissed > 0 && this.status !== "STOPPED") {
        const step = Math.min(remMissed, this.phaseDuration);
        remMissed -= step;
  ...
  updatePhaseStyles() {
    ...
    if (this.status === "WORK") {
      ...
      this.els.status.className =
        "text-sm font-bold tracking-widest uppercase mb-1 primary-text";
      this.els.ring.classList.add("primary-stroke");
    } else if (this.status === "REST") {
      updateText(this.els.status, t("rest"));
      this.els.status.className =
        "text-sm font-bold tracking-widest uppercase mb-1 text-blue-500";
      this.els.ring.style.stroke = "#3b82f6";
    } else {
      updateText(this.els.status, t("get_ready"));
      this.els.status.className =
        "text-sm font-bold tracking-widest uppercase mb-1 app-text-sec";
      this.els.ring.classList.add("primary-stroke");
    }
  },
