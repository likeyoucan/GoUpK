    ...
    (function () {
        ...
        function loadScriptOnce(src) {
            ...
            window.__erudaDockScriptPromises[src] = new Promise(function (resolve, reject) {
  ...
  #__erudaDockBar{\
    ...
    box-sizing:border-box; box-shadow: 0 4px 15px rgba(0,0,0,0.15);\
  }\
  ...
  ";
        ...
        onReady(function () {
            ...
            var toggleUiBtn = el("button", { id: "__erudaDockToggleUi", text: "Hide UI" }, topRow);
            ...
            var sizeLabel = el("div", { id: "__erudaSizeLabel", text: "Size: —" }, sizeRow);
            var sizeInput = el("input", { id: "__erudaSize", type: "range", min: "20", max: "90", step: "1" }, sizeRow);
            ...
            function applyDock() {
                ...
                if (pos === "top" || pos === "bottom") {
                    ...
                    dock.style[pos] = "0"; dock.style.height = sizeV + "vh"; dock.style.width = "100%";
                    sizeInput.value = String(sizeV);
                    sizeLabel.textContent = "Size: " + sizeV + "vh (height)";
                } else {
                    sizeH = clamp(sizeH, 20, 90);
                    dock.style.top = "0"; dock.style.bottom = "0";
                    dock.style[pos] = "0"; dock.style.width = sizeH + "vw"; dock.style.height = "100%";
            ...
            ["top", "bottom", "left", "right"].forEach(function (p) {
                el("button", { text: p.charAt(0).toUpperCase() + p.slice(1), onclick: function () { setPos(p); } }, btns);
            });

            function onSizeInput() {
                var val = Number(sizeInput.value);
                if (pos === "top" || pos === "bottom") { sizeV = val; LS.set("eruda-dock-size-v", sizeV); }
                else { sizeH = val; LS.set("eruda-dock-size-h", sizeH); }
                applyDock();
            }
            sizeInput.addEventListener("input", onSizeInput);
            sizeInput.addEventListener("change", function () { onSizeInput(); bringControlsToFront(); });
            ...
            resizeHandle.addEventListener("pointermove", function (e) {
                ...
                LS.set("eruda-dock-size-v", sizeV); LS.set("eruda-dock-size-h", sizeH);
                applyDock();
            });

            function endDrag() { drag = null; bringControlsToFront(); }
            ...
            function ensureScripts() {
                ...
                return loadScriptOnce(CORE_SRC).then(function () {
                    ...
                    PLUGINS.forEach(function (src) { p = p.then(function () { return loadScriptOnce(src); }); });
                    return p.then(function () { scriptsLoaded = true; });
                });
            }
