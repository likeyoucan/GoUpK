...
export const themeManager = {
  ...
  init() {
    ...
    // ИСПРАВЛЕНО (Fix #2): Применяем загруженный акцентный цвет при инициализации
  ...
  setColor(hex, doScroll = true) {
    ...
    // Скрываем кнопку, если она была, т.к. происходит новое действие
  ...
  setBgColor(hex, doScroll = true) {
    if (!isValidHex(hex)) hex = 'default'; // Безопасность
    // Скрываем кнопку
    colorsManager._hideActionButton();
