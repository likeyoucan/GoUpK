...
export const themeManager = {
  ...
  init() {
    ...
    if (window.Capacitor && window.Capacitor.isNativePlatform()) {
      ...
      if (MaterialYou) {
        MaterialYou.isSupported().then(result => {
          if (result.isSupported) {
            MaterialYou.addListener("colorColorsChanged", (colors) => this.applyMaterialYouColors(colors));
            MaterialYou.getColors().then(colors => this.applyMaterialYouColors(colors)).catch(() => {});
          }
        }).catch(() => {});
      }
    }
    ...
    $("toggle-ms")?.addEventListener("change", (e) => { this.showMs = e.target.checked; safeSetLS("app_show_ms", this.showMs); document.dispatchEvent(new CustomEvent("msChanged", { detail: this.showMs })); });
    ...
    $("vignetteSlider")?.addEventListener("input", (e) => { this.vignetteAlpha = parseFloat(e.target.value); safeSetLS("app_vignette_alpha", this.vignetteAlpha); this.updateVignette(); });
    $("fontSlider")?.addEventListener("input", (e) => this.setFontSize(e.target.value));
    this.themeBtns.forEach((btn) => btn.addEventListener("click", (e) => this.setMode(e.target.getAttribute("data-theme-mode"))));
    this.colorBtns.forEach((btn) => btn.addEventListener("click", (e) => this.setColor(e.target.getAttribute("data-color"))));
    this.bgBtns.forEach((btn) => btn.addEventListener("click", (e) => this.setBgColor(e.target.getAttribute("data-bg"))));
    $("customColorInput")?.addEventListener("input", (e) => this.setColor(e.target.value));
    $("customBgInput")?.addEventListener("input", (e) => this.setBgColor(e.target.value));
  ...
  applySettings() {
    ...
    if ($("customColorInput")) $("customColorInput").value = safeGetLS("theme_color") || DEFAULT_ACCENT_COLOR;
    if ($("customBgInput")) $("customBgInput").value = safeGetLS("theme_bg_color") || "#ffffff";
  },
  ...
  setMode(mode) {
    ...
    this.themeBtns.forEach(b => { b.classList.remove("app-surface", "shadow-sm", "app-text"); b.classList.add("app-text-sec"); });
    ...
    let meta = document.head.querySelector('meta[name="color-scheme"]') || document.createElement("meta");
    meta.name = "color-scheme"; meta.content = isDark ? "dark" : "light";
    document.head.appendChild(meta);
    if (window.Capacitor && window.Capacitor.isNativePlatform()) {
      const { StatusBar } = window.Capacitor.Plugins;
      if (StatusBar) StatusBar.setStyle({ style: isDark ? "LIGHT" : "DARK" }).catch(() => {});
    }
  ...
  setColor(hex) {
    ...
    if (hex === "auto") {
      if (window.Capacitor && window.Capacitor.Plugins.MaterialYou) {
        window.Capacitor.Plugins.MaterialYou.getColors().then(c => this.applyMaterialYouColors(c)).catch(() => this.setColor(DEFAULT_ACCENT_COLOR));
      } else { this.setColor(DEFAULT_ACCENT_COLOR); }
  ...
  updateButtons(btnCollection, hex, customId, isBg) {
    ...
    btnCollection.forEach(b => {
      b.classList.remove("ring-2", "ring-offset-2", "ring-offset-white", "dark:ring-offset-gray-900");
    ...
    pickerWrapper.classList.toggle("ring-offset-2", !found && hex !== "default");
    pickerWrapper.classList.toggle("ring-offset-white", !found && hex !== "default");
    pickerWrapper.classList.toggle("dark:ring-offset-gray-900", !found && hex !== "default");
  },
  ...
  applyBgTheme(hex, isDark) {
    ...
    if (hex === "default") {
      root.style.setProperty("--bg-color", !this.isAdaptiveBg ? (isDark ? "#000000" : "#f3f4f6") : "");
      root.style.setProperty("--surface-color", !this.isAdaptiveBg ? (isDark ? "#1c1c1e" : "#ffffff") : "");
      if (this.isAdaptiveBg) { root.style.removeProperty("--bg-color"); root.style.removeProperty("--surface-color"); }
      return;
    }
    ...
    if (!this.isAdaptiveBg) {
      ...
      if (isDark) root.style.setProperty("--surface-color", `color-mix(in srgb, ${hex}, white 12%)`);
      else root.style.setProperty("--surface-color", l > 85 ? `color-mix(in srgb, ${hex}, black 8%)` : `color-mix(in srgb, ${hex}, white 25%)`);
      document.body.classList.add(this.getLuminance(r, g, b) < 0.5 ? "force-light-text" : "force-dark-text");
      return;
    }
    const satDark = Math.min(s, 40), satLight = Math.max(s, 20);
    root.style.setProperty("--bg-color", `hsl(${h}, ${isDark ? satDark : satLight}%, ${isDark ? 8 : 94}%)`);
    root.style.setProperty("--surface-color", `hsl(${h}, ${isDark ? satDark : satLight}%, ${isDark ? 14 : 98}%)`);
  },
  ...
  updateVignette() {
    ...
    if (!bgElement) return;
    if (vignetteContainer) vignetteContainer.style.display = this.hasVignette ? "flex" : "none";
    bgElement.classList.toggle("has-vignette", this.hasVignette);
    if (this.hasVignette) document.documentElement.style.setProperty("--vignette-alpha", this.vignetteAlpha * 0.3);
  },
  ...
  hexToHSL(H) { const { r: r255, g: g255, b: b255 } = this.hexToRGB(H); let r = r255 / 255, g = g255 / 255, b = b255 / 255; let cmin = Math.min(r, g, b), cmax = Math.max(r, g, b), delta = cmax - cmin; let h = 0, s = 0, l = (cmax + cmin) / 2; if (delta != 0) { if (cmax == r) h = ((g - b) / delta) % 6; else if (cmax == g) h = (b - r) / delta + 2; else h = (r - g) / delta + 4; s = delta / (1 - Math.abs(2 * l - 1)); } h = Math.round(h * 60); if (h < 0) h += 360; return { h, s: +(s * 100).toFixed(1), l: +(l * 100).toFixed(1) }; },
  setFontSize(size) { document.documentElement.style.setProperty("--font-scale", size / 16); if ($("fontSizeDisplay")) $("fontSizeDisplay").textContent = size + " px"; safeSetLS("font_size", size); },
