...
export const themeManager = {
  ...
  _removeActionButton(container) {
    ...
    if (actionBtn) {
      ...
      setTimeout(() => { actionBtn.remove(); }, 300);
    }
  },
  ...
  _handleActionButtonLogic(type, hex) {
    ...
    let activeWrapper = container.querySelector(`.custom-color-wrapper[data-color="${hex}"]`);
    if (!activeWrapper) {
      const picker = isAccent ? $('customColorInput') : $('customBgInput');
      if (picker && picker.value === hex) activeWrapper = picker.closest('.relative');
    }
    ...
    if (actionBtnHTML && activeWrapper) {
      ...
      requestAnimationFrame(() => { actionBtnWrapper.classList.add("is-visible"); });
    }
  },
  ...
  updateColorSelectionUI(type, hex) {
    ...
    if (activeWrapper) {
      ...
      if (activeWrapper.classList.contains("custom-color-wrapper")) {
        ...
        let lum = 0.5;
        if (hex !== "default") lum = this.getLuminance(...Object.values(this.hexToRGB(hex)));
  ...
  applySettings() {
    ...
    this.vignetteAlpha = storedVignetteAlpha !== null ? parseFloat(storedVignetteAlpha) : 0.5;
    ...
    if ($("vignetteSlider")) {
      const closestIndex = this.vignetteLevels.reduce((prev, curr, index) => Math.abs(curr - this.vignetteAlpha) < Math.abs(this.vignetteLevels[prev] - this.vignetteAlpha) ? index : prev, 0);
      $("vignetteSlider").value = closestIndex;
    }
    if ($("vibroSlider")) {
      const levels = [0.5, 0.75, 1, 1.5, 2];
      const vibroValue = parseFloat(safeGetLS("app_vibro_level")) || 1;
      const closestIndex = levels.reduce((prev, curr, index) => Math.abs(curr - vibroValue) < Math.abs(levels[prev] - vibroValue) ? index : prev, 0);
  ...
  bindEvents() {
    ...
    $("toggle-adaptive-bg")?.addEventListener("change", (e) => { document.body.classList.add("is-updating-theme"); this.isAdaptiveBg = e.target.checked; safeSetLS("app_adaptive_bg", this.isAdaptiveBg); this.updateAdaptiveClass(); this.applyBgTheme(this.currentBg, document.documentElement.classList.contains("dark")); requestAnimationFrame(() => document.body.classList.remove("is-updating-theme")); });
    ...
    $("vignetteSlider")?.addEventListener("input", (e) => { sm.vibrate(10, "tactile"); this.vignetteAlpha = this.vignetteLevels[e.target.value]; this.updateVignette(); this.updateSliderLabel("vignetteSlider", "vignette-label", this.vignetteLabels); });
    ...
    $("vibroSlider")?.addEventListener("input", (e) => { sm.vibrate(10, "tactile"); this.updateSliderLabel("vibroSlider", "vibro-label", this.vibroLabels); });
    $("vibroSlider")?.addEventListener("change", (e) => { const levels = [0.5, 0.75, 1, 1.5, 2]; sm.vibroLevel = levels[e.target.value] || 1; safeSetLS("app_vibro_level", sm.vibroLevel); sm.vibrate(50, "strong"); });
    ...
    document.body.addEventListener("click", (e) => { if (e.target.closest("#accent-colors-container")) this.handleColorClick(e, "accent"); if (e.target.closest("#bg-colors-container")) this.handleColorClick(e, "bg"); });
    document.body.addEventListener("input", (e) => { if (e.target.id === "customColorInput") this.setColor(e.target.value, true); if (e.target.id === "customBgInput") this.setBgColor(e.target.value, true); });
  ...
  _internalSetMode(mode, useTransition) {
    ...
    // [ИСПРАВЛЕНИЕ] Синхронизируем значение color picker для фона при смене темы
    ...
    if (useTransition) requestAnimationFrame(() => document.body.classList.remove("is-updating-theme"));
  },
  ...
  handleColorClick(event, type) {
    ...
    if (colorWrapper) {
      ...
      if (existingActionBtn && existingActionBtn.classList.contains('color-action-btn')) {
    ...
    if (actionBtn) {
      ...
      if (action === "delete") {
        ...
        if (colorToDeleteWrapper && colorToDeleteWrapper.classList.contains('custom-color-wrapper')) {
            ...
            setTimeout(() => {
                ...
                isAccent ? this.setColor(newActiveColor, false) : this.setBgColor(newActiveColor, false);
            }, 300);
        }
      } else if (action === "add") {
  ...
  deleteCustomColor(type, color) {
    ...
    if (index > -1) {
      ...
      isAccent ? (this.customAccentColors = customColors) : (this.customBgColors = customColors);
  ...
  resetSettings() { const themeKeys = ["theme_mode", "theme_color", "theme_bg_color", "font_size", "app_adaptive_bg", "app_vignette", "app_vignette_alpha", "app_liquid_glass", "app_hide_nav_labels", "app_ring_width", "app_show_ms", "app_sw_minute_beep"]; themeKeys.forEach(safeRemoveLS); this.applySettings(); },
  applyBgTheme(hex, isDark) { const root = document.documentElement; document.body.classList.remove("force-light-text", "force-dark-text"); if (hex === "default") { if (!this.isAdaptiveBg) { root.style.setProperty("--bg-color", isDark ? "#000000" : "#f3f4f6"); root.style.setProperty("--surface-color", isDark ? "#1c1c1e" : "#ffffff"); } else { root.style.removeProperty("--bg-color"); root.style.removeProperty("--surface-color"); } return; } const rgb = this.hexToRGB(hex); const { h, s, l } = this.hexToHSL(hex); if (!this.isAdaptiveBg) { root.style.setProperty("--bg-color", hex); if (isDark) { root.style.setProperty("--surface-color", `color-mix(in srgb, ${hex}, white 10%)`); } else { const mixArg = l > 90 ? "black 5%" : "white 25%"; root.style.setProperty("--surface-color", `color-mix(in srgb, ${hex}, ${mixArg})`); } const luminance = this.getLuminance(rgb.r, rgb.g, rgb.b); document.body.classList.toggle("force-light-text", luminance < 0.48); document.body.classList.toggle("force-dark-text", luminance >= 0.48); } else { const satDark = Math.min(s, 40); const satLight = Math.max(s, 20); if (isDark) { root.style.setProperty("--bg-color", `hsl(${h} ${satDark}% 8%)`); root.style.setProperty("--surface-color", `hsl(${h} ${satDark}% 14%)`); } else { root.style.setProperty("--bg-color", `hsl(${h} ${satLight}% 94%)`); root.style.setProperty("--surface-color", `hsl(${h} ${satLight}% 98%)`); } } },
