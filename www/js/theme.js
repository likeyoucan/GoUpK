...
export const themeManager = {
  ...
  resetSettings() {
    ...
    // 1. Применяем настройки по умолчанию. Эта функция теперь сама
    // обновит и состояние, и стили страницы, и выбранные элементы в UI.
    this.applySettings();

    // 2. Полностью перерисовываем секции с цветами, чтобы сбросить
    // любые временные состояния (например, показанную кнопку удаления).
    this._populateColorSection("accent");
    this._populateColorSection("bg");

    // 3. Снова обновляем UI выбора цвета, так как populate его пересоздал.
