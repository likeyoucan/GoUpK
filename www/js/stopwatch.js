...
export const sw = {
  ...
  init() {
    ...
    this.els.sessionsList?.addEventListener("click", (e) => {
      const header = e.target.closest(".sw-session-header"),
        renameBtn = e.target.closest(".sw-rename-btn"),
        deleteBtn = e.target.closest(".sw-delete-btn");
  ...
  formatTime(ms, forceMs = null) {
    ...
    return formatMilliseconds(ms, { showMs, showHours: "auto" });
  },
  getUniqueName(baseName) {
    let name = baseName,
      counter = 1;
    while (
      this.savedSessions.some(
  ...
  tick(isBackground = false) {
    ...
    if (now - this.lastRender >= 16 || isBackground) {
      if (!isBackground) this.updateDisplay();
        ...
        updateTitle(
          formatMilliseconds(this.elapsedTime, {
            showMs: false,
            showHours: "auto",
          }),
        );
  ...
  updateDisplay() {
    const timeStr = formatMilliseconds(this.elapsedTime, {
      showMs: themeManager.showMs,
      showHours: false,
    });
  ...
  recordLapOrReset() {
    ...
    if (this.isRunning) {
      ...
      if (this.laps.length === 1) {
        ...
        const prev = this.els.lapsContainer.firstElementChild;
        if (prev) {
          prev.classList.remove("bg-black/5", "dark:bg-white/5");
          prev
            .querySelector(".split-time")
            ?.classList.replace("primary-text", "app-text");
        }
      }
  ...
  updateSaveButtonVisibility() {
    this.els.saveBtn.classList.toggle(
      "hidden",
      this.laps.length === 0 && this.elapsedTime === 0,
    );
  },
  ...
  prepareSaveSession() {
    ...
    let total = this.laps[0]?.total || 0;
    if (this.elapsedTime > total && this.elapsedTime - total > 10)
      sessionLaps.unshift({
        total: this.elapsedTime,
        diff: this.elapsedTime - total,
  ...
  prepareRenameSession(id, e) {
    e?.stopPropagation();
    const session = this.savedSessions.find((s) => String(s.id) === String(id));
    if (session) this.openNameModal("rename", session.name, id);
  },
  openNameModal(action, defaultName, targetId = null) {
    this.nameModalState = {
      action,
      targetId,
      pendingSession: this.nameModalState.pendingSession,
    };
    this.els.nameError?.classList.add("hidden");
  ...
  confirmNameModal() {
    ...
    if (this.nameModalState.action === "save") {
      ...
      const session = this.savedSessions.find(
        (s) => String(s.id) === String(this.nameModalState.targetId),
      );
  ...
  openClearModal() {
    this.els.clearModal.classList.remove("hidden", "opacity-0");
  ...
  toggleSessionDetails(id) {
    ...
    iconEl.style.transform = detailsEl.classList.contains("hidden")
      ? "rotate(0deg)"
      : "rotate(180deg)";
  },
  ...
  renderSavedSessions() {
    ...
    this.savedSessions.forEach((session) => {
      const dateObj = new Date(
        session.date ||
          (typeof session.id === "number" ? session.id : Date.now()),
      );
      ...
      session.laps.forEach(
        (lap, idx) =>
          (lapsHtml += `<div class="flex justify-between items-center py-2 border-b border-gray-500/10 last:border-0 px-2 ${idx === 0 ? "bg-black/5 dark:bg-white/5 rounded-lg" : ""}"><span class="text-xs app-text-sec font-medium">${t("lap_text")} ${lap.index}</span><div class="flex items-center gap-4"><span class="font-mono text-[10px] app-text-sec opacity-60 w-16 text-right">${this.formatTime(lap.total, true)}</span><span class="font-mono text-xs font-bold ${idx === 0 ? "primary-text" : "app-text"} w-16 text-right">${this.formatTime(lap.diff, true)}</span></div></div>`),
      );
      const div = document.createElement("div");
      div.className =
