...
export const sw = {
  ...
  init() {
    this.els = {
      ...
      ring: $("sw-progressRing"), saveBtn: $("sw-saveBtn"),
      sessionsList: $("sw-sessionsList"), sortSelect: $("sw-sortSelect"),
    ...
    this.els.sessionsList?.addEventListener("click", (e) => {
      ...
      if (renameBtn) { e.stopPropagation(); this.prepareRenameSession(Number(renameBtn.dataset.id)); }
      else if (deleteBtn) { e.stopPropagation(); this.deleteSession(Number(deleteBtn.dataset.id)); }
  ...
  toggle() {
    ...
    if (this.isRunning) {
      this.isRunning = false; this.pauseTime = Date.now();
      ...
      this.els.lapBtn.classList.remove("app-surface", "app-text"); this.els.lapBtn.classList.add("bg-red-500", "text-white", "is-reset");
      ...
      this.startTime = performance.now() - this.elapsedTime; this.isRunning = true; this.pauseTime = 0;
      requestWakeLock(); bgWorker.postMessage("start"); this.tick();
      this.els.status.classList.add("hidden"); this.els.display.classList.remove("is-go");
      this.els.lapBtn.classList.remove("hidden"); updateText(this.els.lapBtn, t("lap"));
      this.els.lapBtn.classList.remove("bg-red-500", "text-white", "is-reset"); this.els.lapBtn.classList.add("app-surface", "app-text");
    }
  ...
  updateDisplay() {
    ...
    if (this.els.extendedDisplay) {
      ...
      if (extStr) { updateText(this.els.extendedDisplay, extStr); this.els.extendedDisplay.classList.remove("hidden"); }
      else { this.els.extendedDisplay.classList.add("hidden"); }
    }
    updateTitle(this.formatTime(this.elapsedTime, false));
    if (this.els.ring) this.els.ring.style.strokeDashoffset = this.ringLength - ((this.elapsedTime % 60000) / 60000) * this.ringLength;
  },
  ...
  recordLapOrReset() {
    ...
    if (this.isRunning) {
      ...
      if (this.laps.length === 1) {
        ...
        this.els.currentLapsHeader.classList.remove("hidden"); this.els.currentLapsHeader.classList.add("flex");
      } else {
      ...
      if (this.els.lapFlash) {
        ...
        void this.els.lapFlash.offsetWidth; this.els.lapFlash.classList.add("flash-active");
      }
      this.updateSaveButtonVisibility();
      ...
      this.els.lapsContainer.replaceChildren(); this.els.lapsContainer.insertAdjacentHTML("afterbegin", `<div class="text-center app-text-sec opacity-50 mt-4 text-sm" data-i18n="no_laps">${t("no_laps")}</div>`);
      this.updateSaveButtonVisibility();
    }
  },
  ...
  updateSaveButtonVisibility() {
    ...
    if (this.laps.length > 0) {
      this.els.saveBtn.classList.remove("hidden"); this.els.saveBtn.classList.add("flex");
    } else {
      this.els.saveBtn.classList.add("hidden"); this.els.saveBtn.classList.remove("flex");
    }
  },
  
  prepareSaveSession() {
    if (this.laps.length === 0 && this.elapsedTime === 0) return;
    let sessionLaps = [...this.laps]; let total = this.laps.length > 0 ? this.laps[0].total : 0;
    if (this.elapsedTime > total) {
      const diff = this.elapsedTime - total;
      if (diff > 10) sessionLaps.unshift({ total: this.elapsedTime, diff, index: sessionLaps.length + 1 });
    }
    const defaultName = this.getUniqueName(t("stopwatch"));
    const completionTime = this.isRunning ? Date.now() : this.pauseTime || Date.now();
    const pendingSession = { id: Date.now(), name: "", date: completionTime, totalTime: this.elapsedTime, laps: sessionLaps };
    modalManager.open('sw-name-modal', { action: 'save', name: defaultName, pendingSession });
  },

  prepareRenameSession(id) {
    const session = this.savedSessions.find((s) => s.id === id);
    if (!session) return;
    modalManager.open('sw-name-modal', { action: 'rename', name: session.name, targetId: id });
  },

  prepareNameForm(data) {
    this.nameModalState = { ...data };
    this.els.nameError?.classList.add("hidden");
    updateText(this.els.nameTitle, data.action === "rename" ? t('rename') : t("session_name"));
    this.els.nameInput.value = data.name; this.els.nameInput.placeholder = data.name;
  ...
  confirmNameModal() {
    ...
    const isDuplicate = this.savedSessions.some((s) => s.name.toLowerCase() === finalName.toLowerCase() && (this.nameModalState.action === "save" || s.id !== this.nameModalState.targetId));
    if (isDuplicate) {
      this.els.nameError?.classList.remove("hidden"); this.els.nameInput.classList.add("animate-shake");
      setTimeout(() => this.els.nameInput.classList.remove("animate-shake"), 300); return;
    }
    if (this.nameModalState.action === "save") {
      const session = this.nameModalState.pendingSession; session.name = finalName;
      ...
      if (session) { session.name = finalName; safeSetLS("sw_saved_sessions", JSON.stringify(this.savedSessions)); this.sortSessions(this.currentSort); }
    }
    modalManager.closeCurrent();
  },

  confirmClearAll() {
    this.savedSessions = []; safeRemoveLS("sw_saved_sessions");
    this.renderSavedSessions(); modalManager.closeCurrent(); showToast(t("history_cleared"));
  },

  sortSessions(type) {
    this.currentSort = type; if (this.els.sortSelect) this.els.sortSelect.value = type;
    this.savedSessions.sort((a, b) => {
      if (type === "date_desc") return b.date - a.date; if (type === "date_asc") return a.date - b.date;
      if (type === "name_az") return a.name.localeCompare(b.name); if (type === "name_za") return b.name.localeCompare(a.name);
      if (type === "result_fast") return a.totalTime - b.totalTime; return 0;
    });
  ...
  toggleSessionDetails(id) {
    const detailsEl = $(`sw-details-${id}`), iconEl = $(`sw-icon-${id}`);
    if (!detailsEl) return;
    if (detailsEl.classList.contains("hidden")) {
      detailsEl.classList.remove("hidden"); if (iconEl) iconEl.style.transform = "rotate(180deg)";
    } else {
      detailsEl.classList.add("hidden"); if (iconEl) iconEl.style.transform = "rotate(0deg)";
    }
  },
  ...
  renderSavedSessions() {
    ...
    this.savedSessions.forEach((session) => {
      const dateObj = new Date(session.date || session.id), dateStr = `${dateObj.toLocaleDateString()} ${dateObj.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`;
      ...
      session.laps.forEach((lap, idx) => {
        const isLatest = idx === 0, bgClass = isLatest ? "bg-black/5 dark:bg-black/20 rounded-lg" : "", textColor = isLatest ? "primary-text" : "app-text";
