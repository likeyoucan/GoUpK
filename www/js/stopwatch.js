...
export const sw = {
  ...
  init() {
    ...
    bgWorker.addEventListener("message", (e) => {
      if (e.data === "tick" && this.isRunning && document.hidden) this.tick(true);
    });
  ...
  // ... (методы formatTime, getUniqueName, toggle, tick, updateDisplay, createLapElement, recordLapOrReset, reRenderCurrentLaps, updateSaveButtonVisibility остаются без изменений) ...
  ...
  toggle() { /* ... без изменений ... */ },
  tick(isBackground = false) { /* ... без изменений ... */ },
  updateDisplay() { /* ... без изменений ... */ },
  createLapElement(lap, isLatest = false) { /* ... без изменений ... */ },
  recordLapOrReset() { /* ... без изменений ... */ },
  reRenderCurrentLaps() { /* ... без изменений ... */ },
  updateSaveButtonVisibility() { /* ... без изменений ... */ },

  // --- Модифицированные и новые методы для работы с модалками ---
  ...
  prepareSaveSession() {
    ...
    if (this.elapsedTime > total) {
      ...
      if (diff > 10) {
        sessionLaps.unshift({ total: this.elapsedTime, diff, index: sessionLaps.length + 1 });
      }
    }
