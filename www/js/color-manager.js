...
export const colorManager = {
  ...
  _bindEvents() {
    ...
    $("customColorInput")?.addEventListener("input", (e) => {
      document.dispatchEvent(
        new CustomEvent("colorSelected", {
          detail: { type: "accent", color: e.target.value, fromPicker: true },
        }),
      );
    });
    $("customBgInput")?.addEventListener("input", (e) => {
      document.dispatchEvent(
        new CustomEvent("colorSelected", {
          detail: { type: "bg", color: e.target.value, fromPicker: true },
        }),
      );
    });
  ...
  _showDeleteButton(targetWrapper) {
    ...
    btn.className =
      "delete-color-btn absolute -top-2.5 -right-2.5 w-7 h-7 flex items-center justify-center rounded-full text-white bg-red-500 shadow-lg focus:outline-none custom-focus active:scale-90 transition-all z-20";
  ...
  _hideDeleteButton() {
    ...
    if (btn) {
      btn.style.animation =
        "fadeOutScale 0.2s cubic-bezier(0.4, 0, 0.2, 1) both";
  ...
  addCustomColor(type, color) {
    ...
    if (
      ...
      document.dispatchEvent(
        new CustomEvent("colorSelected", {
          detail: { type, color: normalizedColor, fromPicker: false },
        }),
      );
      return;
    }
    ...
    if (customColors.length >= MAX_CUSTOM_COLORS) {
      ...
      document.dispatchEvent(
        new CustomEvent("revertColorSelection", { detail: { type } }),
      );
      return;
    }
