...
export const colorManager = {
  ...
  addCustomColor(type) {
    ...
    const isAlreadyAdded = customColors
      .map((c) => c.toLowerCase())
      .includes(color);
  ...
  highlightExistingColor(type, color) {
    ...
    const existingSwatch = container.querySelector(
      `.color-swatch-wrapper[data-color="${color.toLowerCase()}"]`,
    );
    if (existingSwatch) {
      existingSwatch.scrollIntoView({
        behavior: "smooth",
        block: "nearest",
        inline: "center",
      });
